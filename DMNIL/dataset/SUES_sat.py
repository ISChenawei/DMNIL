...
class SUES_sat(BaseImageDataset):
    """
    Market1501
    Reference:
    ...
    """
    dataset_dir = 'SUES-200'

    def __init__(self, root, verbose=True, **kwargs):
        super(SUES_sat, self).__init__()
        root='sample4geo/dataset'
        # print('regdb_rgb',trial)
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'Training/150/satellite')
        self.query_dir = osp.join(self.dataset_dir, 'Testing/150/query_satellite')
        self.gallery_dir = osp.join(self.dataset_dir, 'Testing/150/gallery_satellite')
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        img_paths = [p for p in img_paths if osp.splitext(p)[-1].lower() in ['.jpg', '.jpeg', '.png']]  # 过滤支持的扩展名
