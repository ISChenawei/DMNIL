...
class DenseUAV_sat(BaseImageDataset):
    """
    Market1501
    Reference:
    ...
    def __init__(self, root, verbose=True, **kwargs):
        super(DenseUAV_sat, self).__init__()
        root='sample4geo/dataset'
        # print('regdb_rgb',trial)
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'train/satellite')
        self.query_dir = osp.join(self.dataset_dir, 'test/query_satellite')
        self.gallery_dir = osp.join(self.dataset_dir, 'test/gallery_satellite')
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        img_paths = [p for p in img_paths if osp.splitext(p)[-1].strip().lower() in ['.jpg', '.jpeg', '.png', '.tif']]
